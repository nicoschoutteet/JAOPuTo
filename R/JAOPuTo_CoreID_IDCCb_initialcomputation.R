#' CoreIDCC(b) - Initial Computation Computation (Virgin Domain)
#'
#' @description
#' Download pre-final flow-based parameters (early publication) in Core IDCC(b).
#'
...
#' @examples
#' \dontrun{
#' JAOPuTo_CoreID_IDCCb_initialcomputation(
#'   start = "2025-01-01 00:00",
#'   end = "2025-01-01 01:00"
#' )
#' }
JAOPuTo_CoreID_IDCCb_initialcomputation <- function(start, end,
                                                    presolved = TRUE) {
  ...
  df <- JAOPuTo_get(
    dataset  = "coreID",
    endpoint = "api/data/IDCCB_initialComputation",
    start    = start,
    end      = end,
    Filter   = filter_json
