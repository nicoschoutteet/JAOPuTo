#' CoreID - ID1 - Scheduled exchanges
#'
...
#'
#' @return a dataframe containing the scheduled bilateral exchanges between each combination of Core bidding zones
#' @import tibble
#' @import httr
#' @import jsonlite
#' @import dplyr
...
#' @export
#'
#' @examples JAOPuTo_CoreID_ID1_scheduledexchanges(as.POSIXct("2024-07-01 00:00", "Europe/Brussels"),
#' as.POSIXct("2024-07-01 23:00", "Europe/Brussels"))
JAOPuTo_CoreID_ID1_scheduledexchanges <- function(StartDateTime,
                                            EndDateTime) {

  df <- tibble::tibble()

  API_GET <- httr::GET("https://publicationtool.jao.eu/coreid/api/data/ID1_scheduledExchanges",
                       query = list(
  ...
  df <- API_GET$data %>%
    ...
    dplyr::mutate(BiddingZoneAbbFrom = substr(Variable, 8, 9),
                  ...
                  BiddingZoneTo = dplyr::case_match(BiddingZoneAbbTo,
                                                    ...
                                                    "SI" ~ "Slovenia",
                                                    "SK" ~ "Slovakia")) %>%
    dplyr::select(DateTime, BiddingZoneAbbFrom, BiddingZoneAbbTo, BiddingZoneFrom, BiddingZoneTo, ScheduledExchange) %>%
    return()
