#' Core - Maximum Final bilateral exchange restrictions
#'
...
#'
#' @return a dataframe containing the final bilateral exchange restrictions between each combination of Core bidding zones
#' @import tibble
#' @import httr
#' @import jsonlite
#' @import dplyr
...
#' @export
#'
#' @examples JAOPuTo_Core_maxexchangerestrictions(as.POSIXct("2024-01-01 00:00", "Europe/Brussels"),
#' as.POSIXct("2024-01-01 23:00", "Europe/Brussels"))
JAOPuTo_Core_maxexchangerestrictions <- function(StartDateTime,
                                      EndDateTime) {

  df <- tibble::tibble()

  API_GET <- httr::GET("https://publicationtool.jao.eu/core/api/data/bexRestrictions",
                       query = list(
  ...
  df <- API_GET$data %>%
    ...
    tidyr::pivot_longer(-DateTime,
                        names_to = "Variable",
                        values_to = "MaxExchangeRestrictions") %>%
    dplyr::mutate(BiddingZoneAbbFrom = substr(Variable, 8, 9),
                  ...
                  BiddingZoneTo = dplyr::case_match(BiddingZoneAbbTo,
                                                    ...
                                                    "SI" ~ "Slovenia",
                                                    "SK" ~ "Slovakia")) %>%
    dplyr::select(DateTime, BiddingZoneAbbFrom, BiddingZoneAbbTo, BiddingZoneFrom, BiddingZoneTo, MaxExchangeRestrictions) %>%
    return()
