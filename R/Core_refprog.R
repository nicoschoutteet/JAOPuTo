...
#'
#' @return a dataframe containing the exchange data per border that are used for merging of the European grid models including HVDCinterconnectors within the synchronous area.
#' @import tibble
#' @import httr
#' @import jsonlite
#' @import dplyr
...
#' @export
#'
#' @examples JAOPuTo_Core_refprog(as.POSIXct("2024-01-01 00:00", "Europe/Brussels"),
#' as.POSIXct("2024-01-01 23:00", "Europe/Brussels"))
JAOPuTo_Core_refprog <- function(StartDateTime,
                              EndDateTime) {

  df <- tibble::tibble()

  API_GET <- httr::GET("https://publicationtool.jao.eu/core/api/data/refProg",
                       query = list(
