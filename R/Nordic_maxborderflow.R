...
#'
#' @return a dataframe containing the maximum border flow (MaxBFlow) between each combination of Nordic bidding zones
#' @import tibble
#' @import httr
#' @import jsonlite
#' @import dplyr
...
#' @export
#'
#' @examples JAOPuTo_Nordic_maxborderflow(as.POSIXct("2024-10-30 00:00", "Europe/Brussels"),
#' as.POSIXct("2024-10-30 23:00", "Europe/Brussels"))
JAOPuTo_Nordic_maxborderflow <- function(StartDateTime,
                                         EndDateTime) {

  df <- tibble::tibble()

  API_GET <- httr::GET("https://publicationtool.jao.eu/nordic/api/data/maxBorderFlow",
                       query = list(
  ...
  df <- API_GET$data %>%
    ...
    dplyr::mutate(BiddingZoneAbbFrom = stringr::str_split(Variable, "_", simplify = TRUE)[, 2],
                  ...
                  BiddingZoneTo = dplyr::case_match(BiddingZoneAbbTo,
                                                    ...
                                                    "SE4" ~ "Sweden 4",
                                                    "SE4SWL" ~ "Sweden 4 - SWL")) %>%
    dplyr::select(DateTime, BiddingZoneAbbFrom, BiddingZoneAbbTo, BiddingZoneFrom, BiddingZoneTo, MaxBorderFlow) %>%
    return()
