...
#'
#' @return a dataframe containing the long-term nominations on Core bidding zone borders
#' @import tibble
#' @import httr
#' @import jsonlite
#' @import dplyr
...
#' @export
#'
#' @examples JAOPuTo_Core_longtermnominations(as.POSIXct("2024-01-01 00:00", "Europe/Brussels"),
#' as.POSIXct("2024-01-01 23:00", "Europe/Brussels"))
JAOPuTo_Core_longtermnominations <- function(StartDateTime,
                                         EndDateTime) {

  df <- tibble::tibble()

  API_GET <- httr::GET("https://publicationtool.jao.eu/core/api/data/ltn",
                       query = list(
  ...
  df <- API_GET$data %>%
    ...
    dplyr::mutate(BiddingZoneAbbFrom = substr(Variable, 8, 9),
                  ...
                  BiddingZoneTo = dplyr::case_match(BiddingZoneAbbTo,
                                                    ...
                                                    "SI" ~ "Slovenia",
                                                    "SK" ~ "Slovakia")) %>%
    dplyr::select(DateTime, BiddingZoneAbbFrom, BiddingZoneAbbTo, BiddingZoneFrom, BiddingZoneTo, LongTermNominations) %>%
    return()
