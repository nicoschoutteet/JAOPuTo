...
#'
#' @return a dataframe containing the Available Transfer Capacities (ATC) for the intraday timeframe on Core borders
#' @import tibble
#' @import httr
#' @import jsonlite
#' @import dplyr
...
#' @export
#'
#' @examples JAOPuTo_Core_intradayATC(as.POSIXct("2024-01-01 00:00", "Europe/Brussels"),
#' as.POSIXct("2024-01-01 23:00", "Europe/Brussels"))
JAOPuTo_Core_intradayATC <- function(StartDateTime,
                                           EndDateTime) {

  df <- tibble::tibble()

  API_GET <- httr::GET("https://publicationtool.jao.eu/core/api/data/intradayAtc",
                       query = list(
  ...
  df <- API_GET$data %>%
    ...
    dplyr::select(DateTime, BiddingZoneAbbFrom, BiddingZoneAbbTo, BiddingZoneFrom, BiddingZoneTo, Calculation, ATC) %>%
    return()
