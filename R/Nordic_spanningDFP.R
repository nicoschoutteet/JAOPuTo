...
#' @export
#'
#' @examples JAOPuTo_Nordic_spanningDFP(as.POSIXct("2024-10-30 00:00", "Europe/Brussels"),
#' as.POSIXct("2024-10-30 23:00", "Europe/Brussels"))
JAOPuTo_Nordic_spanningDFP <- function(StartDateTime,
                                       EndDateTime) {
  ...
  if(length(API_GET$data > 0)) {
    ...
    df <- API_GET$data %>%
      ...
      dplyr::mutate(DateTime = lubridate::with_tz(as.POSIXct(dateTimeUtc, tz = "UTC",
                                                  ...
                                                  "Europe/Brussels")) %>%
      dplyr::select(DateTime, Type = type, ReferenceMTU = referenceTimeInterval) %>%
      return()
